...
var Panagram = (function () {
	...
	Panagram.prototype.guess = function (user, guess) {
		...
		if (guess.species === this.answer.species) {
			this.room.add('|html|<b>' + hashColor(user.name) + '</b> guessed <b>' + guess.species + '</b>, which was the correct answer! This user has also won 1 ticket!');
			Db('ticket').set(toId(user.userid), Db('ticket').get(toId(user), 0) + 1).get(toId(user.userid));
			this.end();
		} else {
			this.room.add('|html|<b>' + hashColor(user.name) + '</b> guessed <b>' + guess.species + '</b>, but was not the correct answer...');
