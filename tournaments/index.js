...
class Tournament {
	...
	onTournamentEnd() {
		...
		if (this.room.isOfficial && tourSize >= sizeRequiredToEarn) {
			...
			let secondTicket = Math.round(firstTicket / 2);
			...
			if (runnerUp) {
				Db('ticket').set(rid, Db('ticket').get(rid, 0) + secondTicket);
				this.room.addRaw("<b><font color='" + color + "'>" + Tools.escapeHTML(runnerUp) + "</font> has won " +  "<font color='" + color + "'>" + secondTicket + "</font>  for coming second!</b>");
			}
		}
